...
const PatientResponseTab: React.FC<PatientResponseTabProps> = ({
  responses,
}) => {
  if (!responses || responses.length === 0) {
    return (
      <div className="response-container">No patient responses found.</div>
    );
  }
  ...
  return (
    <div className="response-container">
      ...
      <p className="response-subtitle">
        Detailed breakdown of patient's answers to assessment questions
      </p>
                  ...
                  <div className="not-selected-options">
                    {response.NotSelectedOption ? (
                      response.NotSelectedOption.map((option, idx) => (
                        <span key={idx} className="unselected-badge">
                          {option}
                        </span>
                      ))
                    ) : (
